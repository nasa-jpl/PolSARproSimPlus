...
void	drawTree	(Tree *pT, Graphic_Record *pGR, SIM_Record *pZb, Drawing_Record *pD)
{
   ...
   /* Draw the crowns */
   /*******************/
   if(pT->CrownVolume.head != NULL_PTR2CROWN_LIST){
...
void		Forest_Graphic					(PolSARproSim_Record *pPR)
{
   ...
   forest_image_filename	= (char*) calloc (strlen(pPR->pMasterDirectory)+strlen("forest_image.gri")+1, sizeof(char));
   strcpy  (forest_image_filename, pPR->pMasterDirectory);
   strcat  (forest_image_filename, "forest_image.gri");
   ...
   forest_image_filename	= (char*) calloc (strlen(pPR->pMasterDirectory)+strlen("forest_image.bmp")+1, sizeof(char));
   strcpy  (forest_image_filename, pPR->pMasterDirectory);
   strcat  (forest_image_filename, "forest_image.bmp");
