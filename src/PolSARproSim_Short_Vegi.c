...
void		*Image_Short_Veg_Direct_Stems		(void *threadarg)
{
   /* thread variables */
   ...
   pTA->weight_count       = weight_count;
   pTA->weight_average     = weight_average;
   pTA->Sigma0HH           = Sigma0HH;
   pTA->Sigma0HV           = Sigma0HV;
   pTA->Sigma0VH           = Sigma0VH;
   pTA->Sigma0VV           = Sigma0VV;
   pTA->Sigma0_count       = Sigma0_count;
   pTA->AvgShhvv           = AvgShhvv;
   
   pthread_exit(NULL);
}

void     *Image_Short_Veg_Direct_Leaves      (void *threadarg)
{
   /* thread variables */
   Short_Veg_Thread_Arg       *pTA;
   PolSARproSim_Record        *pPR;
...
int		PolSARproSim_Short_Vegetation_Direct_SMP		(PolSARproSim_Record *pPR)
{
   ...
   Short_Veg_Thread_Arg       *threadarg; /* pointer to array of thread arguments   */
   ...
   threadarg = (Short_Veg_Thread_Arg *)calloc(nx,sizeof(Short_Veg_Thread_Arg));
   threads   = (pthread_t *)calloc(nx,sizeof(pthread_t));
   
   /*************************************************************/
   /* Loop over pixel cells and create imaging threads for each */
   /*************************************************************/
   for (i = 0; i < nx; i++) {
      ...
      threadarg[i].pPR        = pPR;
      /* create threads */
      rc=pthread_create(&threads[i], &attr, Image_Short_Veg_Direct_Stems, (void *)&threadarg[i]);
      if(rc){
         printf("Oops! Thread for azimuth line %d was not created in PolSARproSim_Short_Vegetation_Direct_SMP\n", i);
         exit(-1);
      }
   }
   ...
   for (i=0; i<nx; i++) {
      ...
      if(rc){
         printf("ERR: unable to join thread for azimuth line %d in PolSARproSim_Short_Vegetation_Direct_SMP, code: %d\n", i,rc);
         exit(-1);
      }
   }
   /* loop to add results */   
   for (i = 0; i < nx; i++) {
      weight_average += threadarg[i].weight_average;
      weight_count   += threadarg[i].weight_count;
      Sigma0HH       += threadarg[i].Sigma0HH;
      Sigma0HV       += threadarg[i].Sigma0HV;
      Sigma0VH       += threadarg[i].Sigma0VH;
      Sigma0VV       += threadarg[i].Sigma0VV;
      AvgShhvv       = complex_add (AvgShhvv, threadarg[i].AvgShhvv);
      Sigma0_count   += threadarg[i].Sigma0_count;
   }
...
void		*Image_Short_Veg_Bounce_Stems		(void *threadarg)
{
   /* thread variables */
   Short_Veg_Thread_Arg             *pTA;
   PolSARproSim_Record              *pPR;
...
void		*Image_Short_Veg_Bounce_Leaves		(void *threadarg)
{
   /* thread variables */
   Short_Veg_Thread_Arg             *pTA;
   PolSARproSim_Record              *pPR;
...
int		PolSARproSim_Short_Vegetation_Bounce_SMP		(PolSARproSim_Record *pPR)
{
...
#ifndef NO_SHORT_STEMS
   /****************************************/
   /* Stem bounce backscatter contribution */
   /****************************************/
   ...
   /******************************************************/
   /* Loop over pixel cells and create a thread for each */
   /******************************************************/
   for (i = 0; i < nx; i++) {
      xp	= i*deltax + (deltax - Lx)/2.0;
      /* set the thread argument */
      ...
      threadarg[i].pPR        = pPR;
      /* create threads */
      rc=pthread_create(&threads[i], &attr, Image_Short_Veg_Bounce_Stems, (void *)&threadarg[i]);
      if(rc){
         printf("Oops! Thread for imaging stems in azimuth line %d was not created in PolSARproSim_Short_Vegetation_Bounce_SMP\n", i);
         exit(-1);
      }
   }
   /* loop to join the created threads (runs blocking) */   
   for (i=0; i<nx; i++) {
      rc=pthread_join(threads[i], &status);
      if(rc){
         printf("ERR: unable to join thread for imaging stems in azimuth line %d in PolSARproSim_Short_Vegetation_Bounce_SMP, code: %d\n", i,rc);
         exit(-1);
      }
   }
   /* loop to add results */   
...
#ifndef NO_SHORT_LEAVES
   /****************************************/
   /* Leaf bounce backscatter contribution */
   /****************************************/
   ...
   for (i = 0; i < nx; i++) {
      ...
      if(rc){
         printf("Oops! Thread for imaging leaves in azimuth line %d was not created in PolSARproSim_Short_Vegetation_Bounce_SMP\n", i);
         exit(-1);
      }
   }
   /* loop to join the created threads (runs blocking) */   
   for (i=0; i<nx; i++) {
      rc=pthread_join(threads[i], &status);
      if(rc){
         printf("ERR: unable to join thread for imaging leaves in azimuth line %d in PolSARproSim_Short_Vegetation_Bounce_SMP, code: %d\n", i,rc);
         exit(-1);
      }
   }
   /* loop to add results */   
