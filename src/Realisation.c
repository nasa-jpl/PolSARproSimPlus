...
//   
//   Create_Crown		(&c);
//   axis				= Stem_Direction (pT->species, pPR);
...
//   double   l;  /* --RAedit */
//   /* double		l		= Realise_Living_Crown_Depth (pT->species, pT->height); */ /* Original --RAedit */
//   /* The following edit to allow for external defintion of canopy depth */
//   if(pPR->ForestInput_Flag == EXTERNAL_FOREST_DEFINITION) {
//      l     = pT->canopy_depth;
//   } else {
//      l     = Realise_Living_Crown_Depth (pT->species, pT->height);
//   }
//   double		e     = sqrt (pT->radius*pT->radius + l*l);
//   double		beta	= atan2 (pT->radius, l);
//   d3Vector	axis	= Stem_Direction (pT->species, pPR);
...
//   
//   /* The following edit to allow for external defintion of canopy dry depth */
//   /* d1			= Crown_Fractional_Dry_Depth (pT->species, pT->height)*(pT->height-l); */ /* Original --RAedit */
//   if(pPR->ForestInput_Flag == EXTERNAL_FOREST_DEFINITION) {
//      d1        = pT->canopy_dry_depth;
//   } else {
...
//   double   l;
//   /* The following edit to allow for external defintion of canopy depth */ 
//   /* double	l		= Realise_Living_Crown_Depth (pT->species, pT->height); */ /* Original */
//   if(pPR->ForestInput_Flag == EXTERNAL_FOREST_DEFINITION) {
//      l     = pT->canopy_depth;
//   } else {
//      l     = Realise_Living_Crown_Depth (pT->species, pT->height);
...
//   shape				= CROWN_CYLINDER;
//   
//   /* The following edit allows for external defintion of canopy dry depth */ 
//   /* d1       = Crown_Fractional_Dry_Depth (pT->species, pT->height)*(pT->height-l); */ /* Original */
//   if(pPR->ForestInput_Flag == EXTERNAL_FOREST_DEFINITION) {
//      d1          = pT->canopy_dry_depth;
//   } else {
//      d1					= Crown_Fractional_Dry_Depth (pT->species, pT->height)*(pT->height-l);
...
//         phi			+= azimuth_offset;
//         z0			= Polar_Assign_d3Vector (1.0, theta, phi);
//         /* sr			= Primary_Radius (pT->species, pT->height, tb); */  /* --RAedit, see line below               */    
//         sr     = Primary_Radius(pT, pPR, tb);                          /* This to allow for dbh input from file  */
//         er			= sr*pT->Stem.head->end_radius/pT->Stem.head->start_radius;
//         dp			= Primary_Tropism_Factor (pT, pPR);
//         fp			= dp/(2.0*sqrt(1.0-dp*dp));
//         p0			= Primary_Tropism_Direction	(pT, pPR);
...
//   int         Nsections, i_section;
//   double		t0, t1;
//   double		delta_t, tb, tb1;
//   d3Vector    b0, b0_temp, db0;
//   double		theta;
//   double		delta_phi, phi;
//   d3Vector    z0;
//   double		sr, er;
...
//         tb		= t0 + (i_layer+1)*delta_t - drand()*delta_t; //from pine
//         
//         b0			= Branch_Centre (pT->Stem.head, tb);
//         
...
//
//      case POLSARPROSIM_PINE001:       Realise_Global_Primaries		(pT, pPR);	break;
//      case POLSARPROSIM_PINE002:       Realise_Global_Primaries		(pT, pPR);	break;
//      case POLSARPROSIM_PINE003:       Realise_Global_Primaries		(pT, pPR);	break;
//      case POLSARPROSIM_DECIDUOUS001:	Realise_Global_Primaries      (pT, pPR);	break;
//      default:							break;
//   }
//   return;
//}

//void		Realise_DECIDUOUS001_Secondaries	(Tree *pT, PolSARproSim_Record *pPR)
//{
//   Branch		*pPrimary;
...
//}
//
//void		Realise_PINE001_Secondaries	(Tree *pT, PolSARproSim_Record *pPR)
//{
//   Branch		*pPrimary;
//   int         i_primary, Nprimary;
//   d3Vector    b0, b1;
...
//      yp					= Cartesian_Assign_d3Vector (-sin(phi), cos(phi), 0.0);
//      xp					= Cartesian_Assign_d3Vector (-cos(theta)*cos(phi), -cos(theta)*sin(phi), sin(theta));
//      Ns					= Number_of_Secondaries (pT, pPR, primary_length);
//      if (Ns >= 1) {
...
//   switch (pT->species) {
//      case POLSARPROSIM_HEDGE:			break;
//      case POLSARPROSIM_PINE001:		Realise_PINE001_Secondaries			(pT, pPR);	break;
//      case POLSARPROSIM_PINE002:		Realise_PINE001_Secondaries			(pT, pPR);	break;
//      case POLSARPROSIM_PINE003:		Realise_PINE001_Secondaries			(pT, pPR);	break;//
//      case POLSARPROSIM_DECIDUOUS001:	Realise_DECIDUOUS001_Secondaries	(pT, pPR);	break;
//      default:							break;
//   }
//   return;
//}
...
//   long           tertiary_branch_number, n_Tertiary;
//   int            tbn;
...
//#endif
//   
//   /* Image no. of leaves this way instead, copied form Image_Tree_Direct/Bounce(); --RAedit  **/
//   flg_scaling              = Estimate_SAR_Foliage (pT, pPR, &n_Leaves);
//   ln                       = (int)n_Leaves;
//   pT->Foliage.scale_factor = flg_scaling;
//   
...
void		Realise_Tree (Tree *pT, int i, PolSARproSim_Record *pPR)
{
   ...
   //Realise_Global_Crown (pT, pPR); */ /* The original function call */
...
void		Realise_Tree_Crown_Only (Tree *pT, int i, PolSARproSim_Record *pPR)
{
   ...
   Crown_init_list	(&(pT->CrownVolume));
   //Realise_Global_Crown (pT, pPR); /* the original function call */
   /* this function does not rely on the five species */
   Realise_Crown(pT, pPR);
