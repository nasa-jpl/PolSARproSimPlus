...
void  *Direct_Ground_RangeLine   (void *threadarg)
{
...
int		PolSARproSim_Direct_Ground_SMP		(PolSARproSim_Record        *pPR)
{
   ...
   Direct_Ground_Thread_Arg   *threadarg; /* pointer to array of thread arguments   */
   ...
   for (ix=0; ix<nx; ix++) {
      ...
      Sigma0HV       += threadarg[ix].Sigma0HV;
      Sigma0VH       += threadarg[ix].Sigma0VH;
      Sigma0VV       += threadarg[ix].Sigma0VV;
      AvgShhvv       = complex_add (AvgShhvv, threadarg[ix].AvgShhvv);
      Sigma0_count   += threadarg[ix].Sigma0_count;
   }
   /***********************/
   /* Monitor performance */
   /***********************/
